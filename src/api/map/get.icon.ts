...
const getIconProfileById = async (osm_id : string) => {
  try {
        const query    = `osm_id=${osm_id || ""}`
        const response = await apiClient.get<GetIconByIdResponse>(ROUTE_GET_ICON + query)
...
export const useGetIconProfileById = (osm_id :  string) => {
  return useQuery({
