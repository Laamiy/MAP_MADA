...
function buildColorMatch(vegClass: VegClassType , scheme  : ThemeScheme): ExpressionSpecification {
  const pairs = Object.entries(vegClass).flatMap(([vegClass, { color }]) => [vegClass, color])
  return ['match', ['get', 'class'], ...pairs, scheme.esa_vegetation_raw.default] as unknown as ExpressionSpecification
}

function buildOpacityMatch(vegClass : VegClassType): ExpressionSpecification {
  const matchAt = (fn: (o: number) => number) => {
    const pairs = Object.entries(vegClass).flatMap(([vegClass, { opacity }]) => [
      vegClass,
      fn(opacity),
    ])
...
export function getEsaVegetation(scheme: ThemeScheme): CustomLayer[] {

  const vegClasses: VegClassType = {
  ...
  const esa_vegetation_raw: CustomLayer[] = [
    {
      ...
      paint: {
        ...
        'fill-opacity': buildOpacityMatch(vegClasses),
        'fill-antialias': true,
      },
    },
  ];
  return esa_vegetation_raw;
}
