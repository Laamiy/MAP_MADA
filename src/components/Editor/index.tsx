  ...
  return (
    <>
      <Form
        poi={sel}
        onClose={() => setSel(null)}
        onSave={async (tags: any, comment: any) => {
          if (sel.id === 0) {
            // create
            await axios.post(API, { ...sel, tags, comment });
          } else {
            // update
            await axios.put(`${API}/${sel.id}`, {
              version: sel.version,
              tags,
              comment,
            });
  ...
  return (
    <div className="editor-popup">
      ...
      <label>
        ...
        <select
          value={tags.amenity || ""}
          onChange={(e) => setTags({ ...tags, amenity: e.target.value })}
        >
          <option value="">--</option>
      ...
      <details>
        ...
        <textarea
          ...
          onChange={(e) => setTags(JSON.parse(e.target.value))}
        />
      </details>
      <label>
        Comment{" "}
