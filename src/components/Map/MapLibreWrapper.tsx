...
export const MapLibreWrapper: React.FC<MapLibreWrapperProps> = (
  ...
  const [selectedPOI, setSelectedPOI] = useState<PointOfInterestInterface | undefined>(undefined);
 ...
 const attachEditorInteractions = () => {
    ...
    const poiLayers = (poiSourceName) ? style.layers.filter((l): l is any => "source" in l && l.source === poiSourceName).map((l) => l.id) : [];
    console.log("[INFO] : POI layers found:", poiLayers);

    if (poiLayers.length === 0) {
      console.warn('[WARNING] : No POI layers found, using generic click handler');
      mapInstance.on("click", async (e: maplibregl.MapMouseEvent) => {
        if (!editorEnabled) return;

        const features = mapInstance.queryRenderedFeatures(e.point);
        const poiFeature = features.find(
          (f: maplibregl.MapGeoJSONFeature) => f.properties && (f.properties.osm_id || f.properties.amenity || f.properties.shop)
        );

        // If Alt is held, ignore the feature and create new
    ...
    poiLayers.forEach((layerId) => {
      mapInstance.on("click", layerId, async (e) => {
        ...
        // Alt is pressed, stop this handler so the map-level Alt handler takes over
        if (e.originalEvent.altKey) {
          return; 
        }
    ...
    mapInstance.on("click", (e: any) => {
      if (e.originalEvent.altKey && editorEnabled) {
        // Prevent the map from doing other things 
        handleNewPoi(e.lngLat);
      }
    });
  };
...
const handlePoiClick = async (feature: maplibregl.MapGeoJSONFeature, lngLat: Coordinates) => {
    //  pick fields to avoid the MapLibre property flood
    ...
    if (props.tags) {
      const raw = typeof props.tags === "string" ? JSON.parse(props.tags) : props.tags;
      
      // Filter out system junk 
      Object.entries(raw).forEach(([k, v]) => {
        const isJunk = k.startsWith('mapbox') || k.startsWith('osm_') || k === 'version' || k === 'icon_class';
        if (!isJunk) {
    ...
    //Fallback to core columns if tags are empty
    if (Object.keys(parsedTags).length === 0) {
      [
        "name", "amenity", "shop", "tourism", "man_made", "leisure", "natural",
      ].forEach((key) => {
    ...
    try {
      ...
      // Fetch fresh, clean data from the API
...
//----------------------------------------------------------------------------------------
  ...
  const bustTiles = () => {
    ...
    try {
      ...
      Object.keys(style.sources).forEach((sourceName) => {
        ...
        if (src?.tiles) {
          src.tiles = src.tiles.map((t: string) => `${t.split("?")[0]}?v=${Date.now()}`);
  ...
  useEffect(() => {
    ...
    if (route) {
      ...
      const bounds = coordinates.reduce((b, c) => b.extend([c[0], c[1]]),
