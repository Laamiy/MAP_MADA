      ...
      <AnimatePresence mode="wait">
        {value && (
          <motion.div
            ...
            className="absolute !top-full !left-0 !right-0 !mt-3 !w-full !max-h-[380px] overflow-y-auto bg-background/80 border border-border rounded-2xl shadow-2xl z-50 p-2 backdrop-blur-xl origin-top"
          >
            {loading && (
              <motion.div 
                initial={{ opacity: 0 }}
                ...
                {Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="flex items-center !gap-3 rounded-xl !px-3 !py-2.5 w-full">
                    ...
                    <div className="flex flex-col !gap-2 !flex-1 min-w-0">
                      ...
                      <Skeleton className="!h-2.5 !w-full !max-w-[180px] rounded-md" />
                    </div>
                  </div>
                ))}
              </motion.div>
